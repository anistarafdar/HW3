...
#include "MathProblemBankParser.h"
...
// Source file for problem bank CHANGED
std::string BANK = "math_problems.tex";

// Filename for the created test *************** changed
std::string FILENAME = "math_test.tex";

// // Constraints on the problem choice.
int NUM_PROBLEMS = 10; //3b The test must have 10 problems.
...
int main() {
    ...
    std::default_random_engine generator(seed); // is this smart

    std::uniform_int_distribution<int> three_or_four(3, 4); // cool trick
    const int MIN_LONG_PROBLEMS = three_or_four(generator); 
    const int MAX_LONG_PROBLEMS = MIN_LONG_PROBLEMS;
    const int MIN_TOPIC_PROBLEMS = 1;
    const int MAX_TOPIC_PROBLEMS = 2;
    const int MIN_AUTHOR_PROBLEMS = 1;
    const int MAX_AUTHOR_PROBLEMS = 2; //ok
    ...
    for (const auto& p : bank) {
        // downcasting to extract meta data... is there a simpler way? this feels so invovled and not elegant.
        if (const auto* pr = dynamic_cast<const MathProblem*>(p.get())) {
            topicSet.insert(pr->getTopic());
            authorSet.insert(pr->getAuthor()); // i expect
        }
    }
    std::vector<std::string> allTopics(topicSet.begin(), topicSet.end());
    std::vector<std::string> allAuthors(authorSet.begin(), authorSet.end());
    /*
    ...
    */
    ...
    constraints.push_back(std::make_shared<LengthConstraint>(MIN_LONG_PROBLEMS, MAX_LONG_PROBLEMS));
    constraints.push_back(std::make_shared<TopicConstraint>(MIN_TOPIC_PROBLEMS, MAX_TOPIC_PROBLEMS, allTopics));
    constraints.push_back(std::make_shared<AuthorConstraint>(MIN_AUTHOR_PROBLEMS, MAX_AUTHOR_PROBLEMS, allAuthors));
    ...
    // ^^^^^^wtf was i talking about in 3A???^^^^^^^

    FastSelector selector;
    MathTestWriter writer;
