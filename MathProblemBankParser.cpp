...
//ge 
std::vector<std::shared_ptr<IProblem>> MathProblemBankParser::parse(const std::string& filename) {
    std::ifstream file(filename);
    if (!file.is_open()) {
    ...
    while (current_pos != std::string::npos) {
        ...
        // *****************************************
        // this is where the change happesn
        // ***************************************************


        // Pass the clean problem string to the NEW constructor
        problems.push_back(std::make_shared<MathProblem>(rawProblem));
